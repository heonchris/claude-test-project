...
3. **물 컵 줄** — 탭하면 한 컵, **길게 누르면 취소**
4. **오늘 먹은 것** — 사진 썸네일 가로 스크롤 (끼니 + 찍은 시각 표시). 비어 있으면 `아직 비어 있어요. 첫 끼를 남겨볼까요?`
5. **오늘 운동** — 한 줄 요약. 탭하면 수정
6. **오늘의 플랜** — 활성 플랜의 오늘 항목 체크리스트. **플랜이 없으면 이 섹션 자체를 숨김**
7. 우하단 **플로팅 + 버튼**
...
- 날짜를 탭하면 아래에 그날 상세 — 사진 그리드(끼니 + 시각), 물 컵 수, 운동 목록
...
SQLite 하나. 사진은 DB에 넣지 않고 **파일로 저장하고 경로만 기록**한다.
...
**설정 기본값**
`water_goal=8`, `cat_name='나비'`, `reminder_on=0`, `reminder_meal_time='12:30'`, `reminder_water_time='15:00'`, `workout_goal_minutes=30`
...
components/            Cat, CatWall, ProgressRing, WaterCups, Txt, Input, ui …
db/                    schema.ts · index.ts(연결·마이그레이션) · queries.ts
lib/                   dates · progress · planParser · photos · exif · backup · notify
