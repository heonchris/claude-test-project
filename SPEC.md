...
**그림의 원본은 `lib/catArt.ts` 하나다.** 여기서 SVG 문자열을 만들고,
앱(`components/Cat.tsx`)과 미리보기 도구(`tools/render-cat.mjs`)가 같은 함수를 쓴다.
그래서 미리보기와 앱 화면이 어긋날 수 없다. 그림 파일은 쓰지 않는다.

**상태**는 오늘의 기록으로 정해지고 오래 유지된다.

| state | 조건 | 모습 |
|---|---|---|
| `napping` | 오늘 기록 0개 | 바닥에 늘어져 누움, 눈 감음, 꼬리 축 처짐 |
| `awake` | 기록 1개 이상 | 앉아서 정면 응시 |
| `climbing` | 기록 2종 이상 | 두 팔 들고 매달린 자세, 발톱 자국 |
| `top` | 3종 다 + 물 목표 달성 | 꼭대기에 앉아 꼬리 살랑 |

**반응 자세**는 뭔가를 남긴 직후 잠깐만 스쳐간다.

| pose | 언제 | 모습 |
|---|---|---|
| `eating` | 식단 저장 | 밥그릇에 고개 숙이고 까딱까딱 |
| `drinking` | 물 한 컵 | 물컵에 고개 숙이고 물방울 튐 |
| `running` | 운동 저장 | 제자리 뛰기 + 속도선 |
| `startled` | 플랜 체크 | 눈동자 확 커짐 |
| `stretching` | 자다가 첫 기록 | 기지개. 반응 자세 앞에 0.7초 먼저 |

### 3-1-1. 움직임 규칙

**2프레임 방식이다.** 부드럽게 이어붙이지 않고 두 장을 툭툭 바꾼다.
납작한 실루엣에는 이 편이 잘 어울리고 코드도 단순하다.

- 가만히 있을 때: **1.1초**마다 숨쉬기(몸통 ±0.9)와 꼬리
- 반응할 때: **0.32초**마다 빠르게
- 눈 깜빡임: **3.5~7초** 사이 아무 때나 0.12초 (규칙적이면 기계처럼 보인다)
- 화면을 보고 있지 않거나 앱이 백그라운드면 **멈춘다**

> 반복되는 움직임은 아주 작게 유지할 것. 매일 보는 화면에서 크게 흔들리면 일주일이면 질린다.
> 큰 동작은 **사용자가 뭔가 했을 때만**.

모양을 확인하려면 `npm run cat` → `tools/cat-frames.png`에
자세마다 두 프레임과 눈감은 모습이 한 장으로 나온다.
같은 명령이 앱 아이콘과 스플래시도 다시 만든다.
...
lib/                   catArt(고양이 그림) · dates · progress · fasting · planParser · photos · exif · backup · notify
theme/                 colors.ts · typography.ts
tools/                 render-cat.mjs (아이콘·프레임 미리보기 생성)
...
**그림의 원본이 한 곳뿐이다**
`lib/catArt.ts`가 SVG 문자열을 만들고, 앱과 미리보기 도구가 그걸 같이 쓴다.
예전에는 파이썬 스크립트에 도형을 복사해 두어서 어긋날 위험이 있었는데, 그 복사본을 없앴다.
`npm run cat` 하나로 미리보기와 아이콘이 같이 갱신된다.
