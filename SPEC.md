...
| `stretching` | 자다가 첫 기록 | 기지개. 반응 자세 앞에 0.6초 먼저 |
...
기록을 하나 남기면 이 순서로 이어진다. 전부 합쳐 최대 2.7초.

```
(자고 있었으면) 기지개 0.6초 → 반응 자세 1.3초 → (달성률이 올랐으면) 벽 오르기 0.8초 → 원래 상태
```

**반응하는 동안에는 벽이 움직이지 않는다.**
그래야 "밥을 다 먹고 나서 올라간다"처럼 순서대로 보인다.
벽의 높이는 `shownProgress`가 들고 있고, 반응이 끝날 때 실제 달성률로 따라잡는다.
...
- [ ] 기록을 남기면 고양이가 반응하고, 그다음 벽을 올라간다
- [ ] 화면을 보고 있지 않을 때는 고양이가 움직이지 않는다

> 코드로 확인할 수 있는 항목은 `docs/자체점검.md`에 결과를 적어 두었다.
> 폰에서 눈으로 봐야 하는 것(모션의 느낌, 사진이 많을 때의 속도)은 여전히 직접 확인이 필요하다.
