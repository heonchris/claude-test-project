...
function footCard(f, forceOpen) {
  ...
  if (imgs.top || imgs.side) {
    h += `<details${forceOpen ? ' open' : ''}><summary>측정 과정 사진 보기 — 값이 이상할 때 확인용</summary>`;
    if (forceOpen) {
      h += `<div class="note warn" style="margin:8px 0">
        <b>프로그램이 무엇을 &lsquo;발&rsquo;과 &lsquo;종이&rsquo;로 봤는지 아래 그림으로 확인해 주세요.</b><br>
        초록 선이 발 모양을 제대로 따라가지 않았다면 사진 문제입니다.
        그림을 길게 눌러(맥은 마우스 오른쪽 클릭) 저장한 뒤 보내 주시면 원인을 찾아드립니다.</div>`;
    }
